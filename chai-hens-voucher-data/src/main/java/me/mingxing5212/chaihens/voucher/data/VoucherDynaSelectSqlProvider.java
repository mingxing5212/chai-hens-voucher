...
public class VoucherDynaSelectSqlProvider {
    ...
    public String selectVouchers(final VoucherSearchComponent voucherSearchComponent){
        ...
        }}.toString() + "LIMIT " + voucherSearchComponent.getStart() + "," + voucherSearchComponent.getEnd();
    }

    public String selectVouchersCount(final VoucherSearchComponent voucherSearchComponent){
        return new SQL(){{
            SELECT("count(*)");
            FROM("tb_voucher");
            WHERE("1=1");
